...
> **最後更新**：2025-12-17  
> **架構版本**：Phase 3 (Server-Edge-Runner)  
> **狀態**：包含零信任前端原則與 Edge 部署安全

本文件提供 Robot Command Console 系統的安全檢查清單，用於開發、部署和維護階段。

## 核心安全原則

### 🔴 零信任前端 (Zero Trust Frontend)

**原則：所有前端資料視為不可信任，必須在後端完整驗證**

- [x] **所有 API 輸入在後端驗證**（Pydantic models）
- [x] **不依賴前端驗證**（前端驗證僅為 UX）
- [x] **業務邏輯完全在後端執行**
- [x] **權限檢查在後端每個端點執行**
- [x] **假設前端可能被竄改或繞過**
- [x] **Edge 資料同步到 Server 需重新驗證**
- [ ] 實施請求簽名（防止參數篡改）（建議）
- [ ] 前端代碼混淆與完整性檢查（建議）

### 🟠 Edge 環境安全原則

**約束：延遲敏感 (<100ms)、記憶體受限 (4-8GB)、離線運作**
...
- [x] 離線期間使用本地快取
- [ ] 本地資料庫加密（建議）
- [ ] Edge 設備證明 (Device Attestation)（未來）
- [ ] 資源監控與降級策略（建議）
...
- [x] 實施基於角色的存取控制 (RBAC)
- [x] 定義四種角色：admin、auditor、operator、viewer
- [x] **每個操作都在後端檢查權限**（不信任前端）
- [x] 預設使用最低權限角色（viewer）
- [x] **審計日誌僅限 admin/auditor 訪問**（新增）
- [ ] 實施速率限制防止暴力破解（建議）
- [ ] 實施登入失敗鎖定機制（建議）
- [ ] 實施 refresh token 機制（建議）

### 輸入驗證（零信任前端）
- [x] **所有 API 輸入在後端使用 Pydantic 驗證**（不信任前端）
- [x] **不依賴前端驗證**（僅為 UX）
...
- [x] 防止路徑遍歷攻擊（使用絕對路徑）
- [x] **XSS 防護**：使用 `html.escape()` 清理顯示內容
- [x] **假設前端資料可能被篡改**
