...
class TestCloudSyncService(unittest.TestCase):
    ...
    # ==================== sync_approved_commands 異常處理 ====================

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_sync_approved_commands_upload_returns_failure(self, mock_client_class):
        """upload_command 返回 success=False 時應計入 failed"""
        mock_client = Mock()
        mock_client.upload_command.return_value = {'success': False, 'error': 'Duplicate'}
        mock_client_class.return_value = mock_client
        ...
        mock_cmd = Mock()
        mock_cmd.id = 1
        mock_cmd.name = 'Fail Cmd'
        mock_cmd.description = 'desc'
        mock_cmd.category = 'nav'
        mock_cmd.base_commands = '[]'
        mock_cmd.version = 1
        mock_cmd.author_id = 1
        mock_db.query.return_value.filter_by.return_value.all.return_value = [mock_cmd]
        mock_db.query.return_value.get.return_value = mock_user
    ...
    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_sync_approved_commands_upload_raises_exception(self, mock_client_class):
        """upload_command 拋出例外時應計入 failed 並記錄錯誤"""
        import requests as req
        mock_client = Mock()
        mock_client.upload_command.side_effect = req.RequestException("Network error")
        mock_client_class.return_value = mock_client
    ...
    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_sync_approved_commands_db_exception(self, mock_client_class):
        """資料庫查詢拋出例外時應返回空結果"""
        mock_client_class.return_value = Mock()
    ...
    # ==================== download_and_import_command 異常處理 ====================

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_download_and_import_command_download_fails(self, mock_client_class):
        """download_command 返回 success=False 時應返回 None"""
        mock_client = Mock()
        mock_client.download_command.return_value = {'success': False, 'error': 'Not found'}
        mock_client_class.return_value = mock_client
        ...
        result = service.download_and_import_command(999, mock_db, 1)
        assert result is None

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_download_and_import_command_already_exists(self, mock_client_class):
        """指令在本地已存在時應返回現有指令而不重複建立"""
        mock_client = Mock()
        mock_client.download_command.return_value = {
            'success': True,
            'data': {
                'name': 'Existing Cmd', 'description': 'desc',
        ...
        mock_existing_cmd = Mock()
        mock_existing_cmd.name = 'Existing Cmd'
        mock_db = Mock()
        mock_db.query.return_value.filter_by.return_value.first.return_value = mock_existing_cmd
        ...
        result = service.download_and_import_command(42, mock_db, 1)
        assert result == mock_existing_cmd
        mock_db.add.assert_not_called()

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_download_and_import_command_db_commit_fails(self, mock_client_class):
        """DB commit 失敗時應返回 None 並呼叫 rollback"""
        mock_client = Mock()
        mock_client.download_command.return_value = {
            'success': True,
            'data': {
                'name': 'New Cmd', 'description': 'desc',
                'category': 'nav', 'content': '[]', 'version': 1,
                'author_username': 'user'
            }
        }
    ...
    # ==================== restore_user_settings 異常處理 ====================

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_restore_user_settings_raises_exception(self, mock_client_class):
        """download_user_settings 拋出例外時應返回 None"""
        import requests as req
        mock_client = Mock()
        mock_client.download_user_settings.side_effect = req.RequestException("Timeout")
        mock_client_class.return_value = mock_client
        ...
        result = service.restore_user_settings(user_id='user-123')
        assert result is None

    # ==================== browse_cloud_commands 異常處理 ====================

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_browse_cloud_commands_raises_exception(self, mock_client_class):
        """search_commands 拋出例外時應返回空列表"""
        import requests as req
        mock_client = Mock()
        mock_client.search_commands.side_effect = req.RequestException("Connection error")
        mock_client_class.return_value = mock_client
    ...
    # ==================== _dispatch_queued_item 異常處理 ====================

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_dispatch_queued_item_unknown_op_type(self, mock_client_class):
        """_dispatch_queued_item 遇到未知 op_type 應返回 False"""
        mock_client_class.return_value = Mock()
        ...
        result = service._dispatch_queued_item('unknown_op', {'data': 'value'})
        assert result is False

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_dispatch_queued_item_raises_exception(self, mock_client_class):
        """_dispatch_queued_item 遇到 API 例外應捕獲並返回 False"""
        import requests as req
        mock_client = Mock()
        mock_client.upload_user_settings.side_effect = req.RequestException("Network error")
        mock_client_class.return_value = mock_client
        ...
        result = service._dispatch_queued_item(
            'user_settings',
            {'user_id': 'u1', 'settings': {}, 'edge_id': self.edge_id}
        )
        assert result is False

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_dispatch_queued_item_command_history_success(self, mock_client_class):
        """_dispatch_queued_item 處理 command_history 成功案例"""
        mock_client = Mock()
        mock_client.upload_command_history.return_value = {'success': True, 'synced_count': 2}
        mock_client_class.return_value = mock_client
        ...
        result = service._dispatch_queued_item(
            'command_history',
            {'user_id': 'u1', 'records': [{'id': 1}, {'id': 2}], 'edge_id': self.edge_id}
        )
    ...
    # ==================== get_cloud_status 詳情取得失敗 ====================

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_get_cloud_status_details_fail_gracefully(self, mock_client_class):
        """get_cloud_status 當取得詳情失敗時應仍返回基本狀態（不拋出例外）"""
        import requests as req
        mock_client = Mock()
        mock_client.health_check.return_value = True
        mock_client.get_categories.side_effect = req.RequestException("Timeout")
        mock_client.get_featured_commands.side_effect = req.RequestException("Timeout")
        mock_client_class.return_value = mock_client
    ...
    # ==================== flush_queue for command_history ====================

    @patch('Edge.cloud_sync.sync_service.CloudSyncClient')
    def test_flush_queue_sends_command_history(self, mock_client_class):
        """flush_queue 應補發 command_history 類型的快取操作"""
        import requests as req
        mock_client = Mock()
        mock_client.upload_command_history.side_effect = [
            req.RequestException("Connection failed"),
            {'success': True, 'synced_count': 1, 'total': 1}
        ]
        mock_client_class.return_value = mock_client
