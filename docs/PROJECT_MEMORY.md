...
**目標**：完成 CLI 批次操作功能的完整測試驗證與代碼品質檢查。

**實作內容**：

1. **單元測試**（`tests/test_batch_operations.py`）
   - 22 個測試，涵蓋所有核心模組
   - TestBatchModels: 6 個測試（資料模型）
   - TestBatchParser: 8 個測試（多格式解析）
   - TestProgressTracker: 5 個測試（進度追蹤）
   - TestResultExporter: 3 個測試（結果輸出）

2. **整合測試**（`tests/test_batch_integration.py`）
   - 14 個測試，驗證與現有系統整合
   - TestBatchExecutorIntegration: 4 個測試（執行器整合）
   - TestBatchCLIIntegration: 4 個測試（CLI 端到端）
   - TestBatchErrorHandling: 4 個測試（錯誤處理）
   - TestBatchPerformance: 2 個測試（效能測試）
...
@pytest.mark.asyncio
async def test_function(services):  # 會報錯
    pass
...
**經驗教訓**：
1. **避免 async fixtures**：pytest-asyncio 新版對此支援有限
2. **資源管理**：直接在測試函數中建立和清理
3. **簡化測試**：使用乾跑模式避免複雜的服務設置
...
**經驗教訓**：
1. **E402 例外**：sys.path 調整後的導入需要 noqa
2. **批次修正**：使用 sed 批次處理格式問題
3. **分級檢查**：先修正 E/F 級別，W 級別可選
4. **統計報告**：使用 --statistics 了解問題分布
...
**經驗教訓**：
1. **分層測試**：單元 → 整合 → 端到端
2. **乾跑模式**：簡化整合測試，無需真實服務
3. **檔案測試**：使用範例檔案驗證端到端流程
4. **效能測試**：大批次測試（100+ 指令）驗證擴展性
...
**經驗教訓**：
1. **基準設定**：先用乾跑模式建立效能基準
2. **並行測試**：驗證信號量限流是否正常工作
3. **時間斷言**：設定合理的執行時間上限
4. **規模測試**：從 10 → 50 → 100 逐步增加
...
- ✅ 資料模型（創建、轉換、驗證）
- ✅ 批次解析（JSON/YAML/CSV）
- ✅ 執行模式（parallel/sequential/grouped）
- ✅ 進度追蹤（統計、進度條、完成檢查）
- ✅ 結果輸出（JSON/CSV/文字）
- ✅ 錯誤處理（檔案不存在、格式錯誤、驗證失敗）
- ✅ 效能測試（大批次、信號量限流）
- ✅ 端到端（範例檔案驗證）
...
**經驗教訓**：
1. **TDD 實踐**：先寫測試再實作，確保完整覆蓋
2. **測試分類**：單元/整合/效能分開組織
3. **範例驗證**：使用真實範例檔案作為端到端測試
4. **持續驗證**：每次修改後立即運行測試
